...
def main():
    ...
    if drift < 1e-4:
        print(f"  ✓ V2 is transparent at init (drift={drift:.2e}). Growth wiring correct.")
    else:
        print(f"  ✗ V2 NOT transparent (drift={drift:.2e})! Safe-Zero wiring is wrong.")
    ...
    print(f"{'='*72}")
    print(f"{'condition':<18}{'OLD ppl':>10}{'OLD Δ%':>9}{'NEW ppl':>10}{'NEW Δ%':>9}  verdict")
    print(f"{'-'*72}")
    for name, (old_ppl, new_ppl) in results.items():
        ...
        verdict = "PASS" if (old_ok and new_ok) else "—"
        flags   = ("" if old_ok else " OLD↑") + ("" if new_ok else " NEW✗")
        print(f"{name:<18}{old_ppl:>10.3f}{old_chg*100:>+8.2f}%"
              f"{new_ppl:>10.3f}{new_chg*100:>+8.2f}%  {verdict}{flags}")
    print(f"{'='*72}")
    print("  OLD Δ% should be < +1% (preservation).  NEW Δ% should be <= -10% (gain).")
